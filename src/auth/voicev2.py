...
class VoiceAuth(LoggerMixin):
    ...
    def __init__(self, voice_config: dict, logging_level=logging.INFO):
        """
        Initialize the VoiceVerifier with configurable parameters.
        ...
        self._logger = self.setup_logger(__name__)
    ...
    def record_audio(self, filename, duration=5):
        ...
        self._logger.info("Please speak now...")
        ...
        for i in range(duration):
            time.sleep(1)
            self._logger.debug(f"Recording: {i + 1}/{duration} seconds")
    ...
    def extract_voice_code(self, audio_file):
        ...
        try:
            ...
            self._logger.info(f"Extracted voice code: {voice_code}")
            return voice_code
        except sr.UnknownValueError:
            self._logger.error("Speech Recognition could not understand audio")
            return ""
    ...
    def save_enrollments(self, enrollments):
        ...
        self._logger.info(f"Enrollments saved to {self.enrollment_file}")
    ...
    def enroll_user(self, username, wav_file):
        ...
        if not wav_path.exists():
            self._logger.error(f"Error: File {wav_file} does not exist.")
            return False
        ...
        self._logger.info(f"User '{username}' enrolled successfully using file '{wav_file}'.")
        return True
    ...
    def authenticate_user(self, username, duration=5):
        ...
        if username not in enrollments:
            self._logger.info(f"User '{username}' not found in enrollments.")
            return False
        ...
        if not code_match:
            self._logger.info("Voice code does not match.")
            return False
        ...
        self._logger.info(f"Cosine similarity: {similarity:.3f}")

        if similarity > self.threshold:
            self._logger.info("User verified successfully!")
            return True
        else:
            self._logger.info("Voice embedding similarity below threshold. Verification failed.")
            return False
...
if __name__ == "__main__":
    ...
    verifier.enroll_user_with_file(username, wav_file_path)
