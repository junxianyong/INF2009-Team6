...
export default function Mantraps() {
  ...
  const columns = [
    ...
    {title: "Control", dataIndex: "entry_gate_status", render: (status: string, _: GetMantraps["data"][number]) => <Button onClick={() => controlGate(_.id, status === "CLOSED" ? "open": "close")}>{status === "CLOSED" ? "Unlock" : "Lock"}</Button>},
