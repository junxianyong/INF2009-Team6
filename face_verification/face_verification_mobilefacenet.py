...
class FaceVerification():
    ...
    def build_embedding_from_folder(self, base_folder):
        ...
        for person_name in os.listdir(base_folder):
            ...
            print(f"Processing images for person: {person_name}")
            person_embeddings = []
            ...
            if person_embeddings:
                ...
                print(f"Successfully processed {len(person_embeddings)} images for {person_name}")
            else:
                self.logger.warning(f"No valid embeddings generated for {person_name}")

        if embeddings_dict:
            self.save_face_embeddings(embeddings_dict)
            print(f"Saved embeddings for {len(embeddings_dict)} persons")
            return True
...
if __name__ == "__main__":
    ...
    def detect_and_capture_face(face_verifier, output_folder, person_name=None):
        ...
        print(f"Initializing camera {face_verifier.camera_id}...")
    ...
    while True:
        ...
        if img_path:
            ...
            print("Building face embeddings from images...")
            face_verification.build_embedding_from_images([img_path], name)

    print("Running face verification demo...")
